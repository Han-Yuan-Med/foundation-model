...
# Per request by foundation model
train_set_cls = Lung_cls_imagenet(csv_file=train_csv, img_dir=image_path)
train_loader_cls = torch.utils.data.DataLoader(train_set_cls, batch_size=1, shuffle=False)

cls_model = torchvision.models.densenet121(weights=torchvision.models.DenseNet121_Weights.IMAGENET1K_V1)
cls_model.to(device)
cls_model.eval()
...
with torch.no_grad():
    for data in tqdm(train_loader_cls):
        images, labels = data[0].float().to(device), data[1].cpu().numpy()
        # if len(images.shape) > 3:
        #     feature_list.append(np.zeros(1024))
        # else:
        #     images = images.unsqueeze(1)
        feature_list.append(Pool(cls_model.features(images)).squeeze().cpu().detach().numpy())
...
for idx in range(2, 51):
    ...
    if idx == 2:
        train_csv.iloc[np.concatenate([id_array])]. \
            to_csv(f"dataset_initialization\\train_imagenet_{idx}.csv", index=False)
    if idx > 2:
        train_csv_past = pd.read_csv(f"dataset_initialization\\train_imagenet_{idx-1}.csv")
        pd.concat([train_csv_past, train_csv.iloc[np.concatenate([id_array])]]).drop_duplicates().\
            reset_index(drop=True).to_csv(f"dataset_initialization\\train_imagenet_{idx}.csv", index=False)

for idx in range(10, 101, 10):
    train_csv_tmp = pd.read_csv(f"dataset_initialization\\train_imagenet_50.csv").iloc[:idx, :]
    ...
    for epoch in range(epoch_num):
        ...
        if prc_valid > past_prc:
            ...
            PATH = "D:\\PTX Active Learning Nov\\model_classification" + \
                   f"\\VGG11-Data-ext-imagenet-{idx}.pt"
...
results_df.to_csv("D:\\PTX Active Learning Nov\\VGG11-imagenet-10.csv", index=False, encoding="cp1252")
