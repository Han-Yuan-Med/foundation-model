...
train_set_cls = Lung_cls_remedis(csv_file=train_csv, img_dir=image_path)

module = hub.load('D:\\medical-ai-research-foundations-a-repository-of-medical-foundation-models-1.0.0\\'
                  'cxr-50x1-remedis-m')
avg_pool_2d = tf.keras.layers.AveragePooling2D(pool_size=(14, 14))

feature_list = []
with torch.no_grad():
    for data in tqdm(train_set_cls):
        images, labels = data[0], data[1]
        features = avg_pool_2d(module(tf.convert_to_tensor(np.expand_dims(images, axis=0), dtype=tf.float32)))
...
for idx in range(2, 51):
    ...
    if idx == 2:
        train_csv.iloc[np.concatenate([id_array])]. \
            to_csv(f"dataset_initialization\\train_remedis_{idx}.csv", index=False)
    if idx > 2:
        train_csv_past = pd.read_csv(f"dataset_initialization\\train_remedis_{idx-1}.csv")
        pd.concat([train_csv_past, train_csv.iloc[np.concatenate([id_array])]]).drop_duplicates().\
            reset_index(drop=True).to_csv(f"dataset_initialization\\train_remedis_{idx}.csv", index=False)

for idx in range(10, 101, 10):
    train_csv_tmp = pd.read_csv(f"dataset_initialization\\train_remedis_50.csv").iloc[:idx, :]
    ...
    for epoch in range(epoch_num):
        ...
        if prc_valid > past_prc:
            ...
            PATH = "D:\\PTX Active Learning Nov\\model_classification" + \
                   f"\\VGG11-Data-ext-remedis-{idx}.pt"
...
results_df.to_csv("D:\\PTX Active Learning Nov\\VGG11-remedis-10.csv", index=False, encoding="cp1252")
